import { FaFolder, FaCopy, FaCircleDown, FaFile, FaQuestion } from 'react-icons/fa6';
    ...
    <div className="flex justify-between p-3 border rounded-2xl mb-1 border-gray-600 items-center">
      <div className="flex items-center min-w-[150px] space-x-4">
        <div className="w-6 h-6 flex items-center justify-center">
          ...
          {type !== 'file' && type !== 'folder' && <FaQuestion className="h-6 w-6 text-gray-400" />}
        </div>
        <div>
          {type === 'folder' && <a className="text-white" href={newpath}>{name}</a>}
          {type === 'file' && <a className="text-white" href={ddl}>{name}</a>}
          {type !== 'file' && type !== 'folder' && <a className="text-white" href={name}>{name}</a>}
        </div>
      </div>
